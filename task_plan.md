...
#### 批次 5：导入导出收尾（✅ 已完成，模块收官，待用户验收）
- [x] JSON 双向（导出全量备份/导入去重合并；例外经 id 映射接回）
- [x] ICS 导出（VEVENT 含 RRULE/EXDATE/编辑型独立 VEVENT；VTODO 含 DUE/STATUS）
- [x] 导入按钮支持 .ics/.json 自动识别；导出菜单 + 保存对话框
- [x] 往返单测（ICS 导出→再解析、JSON 导出→导入→重复导入全跳过）；cargo 110/零警告、vitest 41、tsc/build 通过
- [ ] 用户手动验收（交付验收清单）
- **状态：** complete
