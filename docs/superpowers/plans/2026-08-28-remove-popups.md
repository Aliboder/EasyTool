...
**目标**：删除 3 个模块弹窗及其全部配套，全局热键精简为单一主窗口热键，保留跟手粘贴（规格见 `docs/superpowers/specs/2026-08-28-remove-popups-design.md`）
...
| 1.3 | 三模块 `mod.rs`（clipboard/search/timetracker） | 删 `POPUP_WINDOW_LABEL`/`ensure_popup_window`/`on_hotkey` | check 通过 |
| 1.4 | `clipboard/paste.rs` | 删 popup 窗口分支，只隐藏主窗口 | check 通过 |
| 1.5 | 三模块 `manifest.json` + quota | 删 `hotkey` 字段；clipboard/search 删 `follow_mouse` | `cargo test` 66 passed |

## Phase 2 · 前端（每步 `npx tsc --noEmit`）

| # | 文件 | 改动 | Done |
|---|------|------|------|
| 2.1 | 删除：`src/{clipboard_popup,search_popup,timetracker_window}.tsx` + 根 3 个 html + `src/lib/popup-entry.tsx` + `src/hooks/usePopupGeometry.ts` + `src/modules/{search,timetracker}/Popup.tsx` | 无 | tsc 通过 |
...
| 2.3 | `ClipSettings.tsx`/`SearchSettings.tsx`/`timetracker/Settings.tsx` + 各 `config.ts` | 删热键、跟随鼠标、重置弹窗尺寸设置项及字段 | tsc 通过 |
