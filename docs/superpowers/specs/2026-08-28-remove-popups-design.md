...
剪贴板 / 搜索 / 时长统计三个模块各有独立弹窗（跟随鼠标、失焦隐藏、延迟创建）。用户决定**完全移除弹窗，只保留主窗口**作为唯一窗口形态，并同步精简全局热键（删除模块独立热键，只留主窗口呼出热键）。

### 1.2 目标

- 删除 3 个弹窗窗口及其全部配套（Rust 建窗/定位、前端入口/挂载/几何记忆、构建入口、权限声明、弹窗配置项）
- 只保留主窗口（沿用现有「面板」行为）作为唯一窗口
- 删除模块独立热键，全局热键只保留「主窗口呼出」（可自定义）；删除「统一/独立模式」开关
- **保留**剪贴板跟手粘贴能力（改为隐藏主窗口回原窗口注入）
...
通用机制（Rust `lib.rs`）：`ensure_popup_window`（延迟创建+恢复 popup_size）、`show_popup_at`（定位+显示）、`popup_position_physical`（Win32 物理坐标+显示器钳制，**主窗口跟随鼠标也在用**）、`on_window_event` 弹窗失焦隐藏分支、hotkey handler 3 个模块分支。各模块 `POPUP_WINDOW_LABEL` + `ensure_popup_window` + `on_hotkey`。前端 `popup-entry.tsx`（主题跟随+挂载）、`usePopupGeometry`（popup_size/fixed_pos 记忆）、组件 `popup` prop 分支。构建：vite MPA 3 入口 + 3 个根 html + capabilities windows 声明。quota 无弹窗，不受影响。
...
- 删除：3 个模块热键、`reapply_hotkeys` 的多分支、模块设置里的热键录制项、manifest/config 模块 hotkey 字段
- 模块启停不再影响热键：`set_module_enabled` 不再调 `reapply_hotkeys`；`set_module_config` 的 hotkey 重注册判断删除（timetracker 的 `reapply_config` 保留）

### 3.3 跟手粘贴（保留，改隐藏目标）

- 剪贴板粘贴（`clipboard/paste.rs`）：把「隐藏剪贴板弹窗」改为「隐藏主窗口」（若可见）→ 等 100ms 焦点回原窗口 → Ctrl+V
- `record_foreground_state` 保留：托盘/热键呼出主窗口时记录前台窗口（现有 `toggle_main` / hotkey main 分支已有调用）

## 4. 删除清单（文件级）

**Rust**
- `src-tauri/src/lib.rs`：`ensure_popup_window`、`show_popup_at`、on_window_event 弹窗失焦分支、hotkey handler 3 个模块分支、`unified_hotkey` 相关（config 字段、`set_unified_hotkey`、`apply_main_window_mode` 简化）、`ResolvedHotkeys` 精简、`read_hotkeys` 精简。`popup_position_physical` **保留**（主窗口跟随鼠标使用）
- 三模块 `mod.rs`：删 `POPUP_WINDOW_LABEL`、`ensure_popup_window`、`on_hotkey`（模块内不再有热键/建窗）
- `clipboard/paste.rs`：隐藏目标改主窗口
- `src-tauri/src/config.rs`：`AppConfig.unified_hotkey`、`hotkeys` 里非 main 项、模块默认配置里的 `hotkey`；`set_unified_hotkey` 命令删除
- 三模块 manifest `default_config` 的 `hotkey`（clipboard/search/timetracker）、quota 的 `hotkey:""`、clipboard/search 的 `follow_mouse`

**前端**
- 入口：`src/{clipboard_popup,search_popup,timetracker_window}.tsx` + 根 3 个 html
- 工具：`src/lib/popup-entry.tsx`、`src/hooks/usePopupGeometry.ts`
- 弹窗组件：`src/modules/search/Popup.tsx`、`src/modules/timetracker/Popup.tsx`（删）
...
**构建/权限**
- `vite.config.ts`：`rollupOptions.input` 删 3 个弹窗入口
- `src-tauri/capabilities/default.json`：`windows` 数组删 3 个弹窗 label
- `src-tauri/tauri.conf.json`：无弹窗窗口声明（main 唯一，无需改）

**文档**
- `AGENTS.md`：窗口节（删 3 弹窗/延迟创建/统一 helper，改唯一主窗口面板+跟手粘贴）、全局热键节（只留主热键、删统一/独立模式）、目录结构（删弹窗入口）、当前模块描述（删「独立弹窗」字样）
...
6. 搜索：复制路径/打开文件联动正常；时长统计：主窗口内查看正常
7. vite 产物与 capabilities 无任何弹窗入口/窗口声明
8. 老用户 config.json 直接升级：启动正常、无报错（残留键被忽略）