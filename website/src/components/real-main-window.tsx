...
const FILES = [
  ...
  { icon: FileText, name: "EasyTool_0.4.4_x64-setup.exe", path: "D:\\Downloads", size: "8.2 MB", time: "2026-08-20", type: "doc" },
];
  ...
  return (
    <div className="flex h-[420px] flex-col overflow-hidden rounded-xl border border-white/10 bg-zinc-900 font-sans text-zinc-100 shadow-2xl shadow-black/40">
      ...
      <div className="flex items-center gap-1 border-b border-white/5 px-3 py-2">
        {FILTERS.map((f, i) => (
          <button key={f} className={`rounded-md px-2 py-1 text-[11px] transition-colors ${i === 0 ? "bg-emerald-500/15 text-emerald-400" : "text-zinc-500 hover:text-zinc-300"}`}>
            {f}
          </button>
        ))}
      </div>
        ...
        <div className="flex flex-wrap content-start gap-2 p-3">
          {FILES.map((f, i) => (
            <button
              key={f.name}
              onClick={() => setSel(i)}
              className={`flex w-[72px] flex-col items-center gap-1 rounded-md border p-1.5 transition-colors ${i === sel ? "border-emerald-500 bg-emerald-500/10" : "border-white/5 hover:border-white/10"}`}
            >
              <div className="flex size-10 items-center justify-center rounded bg-zinc-800">
