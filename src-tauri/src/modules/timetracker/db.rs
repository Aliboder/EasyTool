...
use super::models::{
    auto_categorize, AppListItem, AUTO_CATEGORIZE_VERSION, CategoryBreakdown, CategoryRule,
};
...
// （pub(crate)：db_stats.rs 统计聚合子模块复用）
pub(crate) fn day_start(date: &str) -> String {
    format!("{date} 00:00:00")
}

pub(crate) fn next_day_start(date: &str) -> Option<String> {
...
pub struct TimetrackerDb {
    /// 并发访问由上层 Mutex<TimetrackerState> 串行化；子模块（db_stats）跨文件 impl 需要访问
